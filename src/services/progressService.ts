import type { YearlyData, ProgressItem, Update } from '../types/index';
...
calculateOverallProgress(year: number): number {
  ...
  const totalItems = yearData.items.reduce((sum, item) => sum + item.total, 0);
  const completedItems = yearData.items.reduce((sum, item) => sum + item.completed, 0);
...
export const fetchProgressData = async (): Promise<any> => {
